...
export default function TaskCalendar() {
  ...
  const filterTasksByDate = (selectedDate: Date) => {
    ...
    // Filtrar tareas que iniciaron o finalizaron en esta fecha
  ...
  const getTileContent = ({ date, view }: { date: Date; view: string }) => {
    ...
    if (tasksCount === 0 && subtasksCount === 0) return null;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Calendario de Actividades</h2>
      ...
      <div className="bg-white rounded-lg shadow p-6 max-h-[600px] overflow-y-auto">
        <h2 className="text-xl font-bold text-gray-900 mb-4">
          Actividad del {date.toLocaleDateString("es-CL", {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
        ...
        {selectedDateTasks.length === 0 && selectedDateSubtasks.length === 0 ? (
            ...
            {selectedDateTasks.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-3">
                  Tareas ({selectedDateTasks.length})
                </h3>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-800 font-medium mt-1">
                            De la tarea: <Link href={`/admin/tasks/${subtask.taskId}`} className="text-blue-600 hover:text-blue-800">{subtask.taskTitle}</Link>
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-2 text-xs text-gray-800 font-medium">
