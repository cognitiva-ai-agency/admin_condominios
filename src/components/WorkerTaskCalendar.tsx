...
export default function WorkerTaskCalendar({ userId }: WorkerTaskCalendarProps) {
  ...
  const filterTasksByDate = (selectedDate: Date) => {
    ...
    // Filtrar tareas que iniciaron, finalizaron o están programadas para esta fecha
    const tasksOnDate = tasks.filter((task) => {
      // Tareas que iniciaron en esta fecha
      if (task.actualStartDate) {
        const startDate = new Date(task.actualStartDate).toDateString();
        if (startDate === dateStr) return true;
      }

      // Tareas que finalizaron en esta fecha
    ...
    // Obtener subtareas completadas en esta fecha por el trabajador
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-bold text-gray-900 mb-4">Mi Calendario de Tareas</h2>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2">
          ...
          <div className="mt-4 text-sm text-gray-800 font-medium space-y-1">
            ...
            <p>• Número verde: Subtareas que completaste</p>
          </div>
        </div>
        ...
        scheduledTasks.length === 0 ? (
          ...
          <div className="space-y-6">
            ...
            {selectedDateSubtasks.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-3">
                  Subtareas que Completaste ({selectedDateSubtasks.length})
                </h3>
